...
mod web_ui {
    ...
    fn check_for_wasm_opt() -> bool {
        match Command::new("wasm-opt").arg("--help").spawn() {
            ...
            Err(e) => {
                println!(
                    "cargo:warning=Failed to execute wasm-opt: {e}. WASM will not be optimized"
                );
                false
            }
        }
    ...
    pub fn create_web_tar(profile: &str, build_dir: &Path, workspace_root: &Path) {
    ...
    #[cfg(feature = "web-ui")]
    {
        ...
        if std::env::var("TARGET").unwrap() != "wasm32-unknown-unknown" && !is_doc_build {
            ...
            if !cargo_lock_existed_at_crate_root && crate_root_cargo_lock.exists() {
                std::fs::remove_file("Cargo.lock").unwrap();
            }
        }
    }
}
