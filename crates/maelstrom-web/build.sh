#!/bin/bash

set -ex

profile=$1
build_dir=$2

target=wasm32-unknown-unknown
pkg_dir=$build_dir/$profile/wasm_pkg/
mkdir -p $pkg_dir

cargo build --lib --target $target --profile $profile

wasm-bindgen \
    $build_dir/$target/$profile/maelstrom_web.wasm \
    --out-dir $pkg_dir \
    --typescript \
    --target web

if [ "$profile" = "wasm_release" ]; then
    wasm-opt \
        $pkg_dir/maelstrom_web_bg.wasm \
        -o $pkg_dir/maelstrom_web_bg.wasm-opt.wasm \
        -O

    mv $pkg_dir/maelstrom_web_bg.wasm-opt.wasm $pkg_dir/maelstrom_web_bg.wasm

    # Add this file so we can tell we ran the optimizations
    touch $pkg_dir/.wasm-opt
fi

cp www/* $pkg_dir
tar --create --directory $pkg_dir . --file $build_dir/$profile/web.tar
